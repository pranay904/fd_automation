...
class CreateCYOEarringOrderPOS:
    ...
    def select_product_type(self):
        ...
        product_type["Cyo_Earring"].click()
    ...
    # NEW METHOD
    def select_back_type(self):
        back_type_input = self.page.locator("(//div[@class='v-field__input']//input)[3]")
        back_type_input.click(force=True)

        back_options = self.page.locator(
            "//div[contains(@class,'v-overlay-container')]//div[@role='option']"
        )

        back_options.first.wait_for(state="visible")
        back_options.first.click(force=True)

        selected_value = back_type_input.input_value()
        print(f"Selected back type: {selected_value}")
